...
def load_lora(self, lora_path: str) -> str:
    ...
    try:
        ...
        # If PEFT injection partially mutated decoder, restore clean base decoder.
        try:
            import copy
            if self._base_decoder is not None:
                ...
                self.model.decoder = self.model.decoder.to(self.device).to(self.dtype)
                self.model.decoder.eval()
                logger.info("Restored base decoder after LoRA load failure")
        except Exception:
            logger.exception("Failed to restore base decoder after LoRA load failure")

        # Ensure runtime state stays consistent after failure.
